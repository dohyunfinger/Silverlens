...
**사투리와 생활 언어, 사진, 개인 건강정보를 함께 이해해 시니어에게 쉬운 건강 식생활 정보를 제공하는 AI 서비스**

SilverLens는 “어르신이 기술을 배우는 것”이 아니라 “기술이 어르신의 사용 방식에 맞추는 것”을 목표로 합니다. 큰 글씨·큰 버튼·음성·사진 중심으로 바로 질문할 수 있으며, 건강정보와 대화는 브라우저에 저장하고 파일로 백업할 수 있습니다.
...
| 상태 | 저장·공유 방식 |
| --- | --- |
| 시니어 정보 | 프로필, 건강정보, 메모, 대화는 브라우저 IndexedDB에 저장 |
| 백업·복원 | 데이터 화면에서 JSON 저장 파일을 내보내거나 다시 불러오기 |
| 사진 첨부 | 보내기 전 사진은 브라우저에 임시 보관하며 만료 후 삭제; 질문 시 Gemini API로 전송 |

브라우저 저장소를 지우거나 기기를 바꾸면 로컬 정보가 사라질 수 있습니다. 데이터 화면의 저장 파일 내보내기를 백업 수단으로 사용할 수 있습니다.
...
│   └── drizzle/0002_mfds_pill_catalog.sql # 식약처 의약품 카탈로그 D1 마이그레이션
├── app/
│   ├── api/
│   │   ├── chat/                          # 시니어 AI 답변
...
│   │   ├── geminiService.ts               # 시니어 답변과 안전 하한선
│   │   ├── transcriptionService.ts        # 음성 인식과 건강정보 추출
│   │   ├── ttsService.ts                  # 답변 음성 생성·캐시
│   │   └── mfdsPillData.ts                # 식약처 의약품 카탈로그 조회·동기화
...
| 변수 | 용도 |
| --- | --- |
| `GEMINI_API_KEY` | 시니어 대화, 음성 인식, TTS |
