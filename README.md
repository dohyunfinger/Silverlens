...
- DATA는 요청마다 검색되는 참고자료입니다.
- 벡터 검색(RAG)이 아닌 문자열·점수 기반 검색입니다.
- 최종 안전성 판단은 Gemini 생성 결과입니다.
- 의료 위험도 계산 엔진은 없습니다.
- 사진 기반 DATA 검색은 아직 완전히 연결되지 않았습니다.
- 질병·알레르기 추출은 Gemini 기반입니다.
- 성별은 현재 AI 응답 생성에 반영되지 않습니다.
- 언어는 요청에는 포함되지만 Gemini 답변 생성에는 직접 활용되지 않습니다.
- Cloudflare D1과 Drizzle은 현재 핵심 질의응답 경로에서 직접 사용되지 않습니다.
