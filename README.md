...
대화 API는 식품·식재료·조리법·보관·영양·식품 알레르기처럼 서비스 범위에
해당하는 질문만 답변합니다. 레이싱 선수, 정치, 금융, 연예 등 명확한 범위 밖
질문은 서버 규칙으로 먼저 차단하고, 그 밖의 질문은 Gemini의 구조화된
`in_scope` 판정 결과를 서버가 다시 확인합니다. 범위 밖 질문에는 해당 주제의
정답을 노출하지 않고 선택 언어에 맞는 식품 질문 안내문을 반환합니다.
