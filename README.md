...
- 답변 음성은 조각 단위로 필요할 때 만듭니다. 첫 조각만 미리 준비하고, 나머지는
  재생 중에 이어서 요청합니다.
- 속도를 기본값(보통)으로 둔 상태에서는 서버 TTS를 아예 호출하지 않습니다.
- 같은 문장 음성은 서버 메모리(30분, 20MB 상한)와 브라우저 캐시에 보관합니다.
- 설정 안내 음성은 문장이 고정이라 메모리와 Cache API에 저장해 재사용합니다.

## 답변 음성 지연 줄이기

Gemini TTS는 글자 수에 거의 비례해 생성 시간이 늘어납니다(실측 ko-KR).

| 요청 길이 | 생성 시간 | 만들어진 오디오 |
|---|---|---|
| 19자 | 4.9초 | 4.4초 |
| 59자 | 8.1초 | 11.8초 |
| 178자 | 20.3초 | 33.1초 |
...
서비스 화면(어르신용)과 소개 화면(가족·심사용)은 목적이 달라 밀도가 다릅니다.
다만 **브랜드 언어는 공유하고 밀도만 다르게** 두어 같은 서비스로 읽히게 합니다.

공유하는 것 (`app/globals.css`의 `:root`)

| 토큰 | 값 | 용도 |
|---|---|---|
| `--r-xs` ~ `--r-xl` | 8 / 12 / 16 / 20 / 28px | 모서리 반경은 이 계단에서만 고른다 |
| `--fs-100` ~ `--fs-500` | 14 / 16 / 18 / 21 / 26px × `--fs-scale` | 글자 크기 계단 |
| `--fs-scale` | 소개 1.0 · 서비스 1.18 | 같은 계단의 확대판 관계 |
| `--brand-dark-top/bottom` | `#24584e` / `#153d34` | 서비스 사이드바와 소개 CTA의 공통 다크 그린 |
| 포커스 링 | `4px #60a5fa` | 두 화면 동일 |

다르게 두는 것

- 서비스: 크림 배경, 2px 이상 두꺼운 테두리, 48px 이상 터치 타깃, 반경 8~20px
- 소개: 흰 배경, 그림자 기반 깊이, 큰 여백, 반경 20~28px

소개 화면 색은 `.about-root`에 지역 변수로 모여 있습니다. 본문 대비는 흰 배경
기준 4.5:1 이상을 계산해 확보했습니다(`--about-muted` 7.8:1, `--about-soft` 5.5:1,
`--about-secondary` 5.2:1). 정확한 접근성 판정은 실제 스크린리더와 확대 환경에서
확인이 필요합니다.

마지막 CTA 섹션은 착지 구간입니다. 딥그린에서 시작해 팀 소개 블록을 서비스
화면과 같은 크림 카드로 두고, 페이지 끝을 서비스 배경색(`#eee7d8`)으로 흘려
"여기서부터 서비스"임을 미리 알립니다. 핵심 기능 섹션의 `.about-preview`는
실제 서비스 UI 조각(위험도 칩, 사투리 변환, 큰 글자 답변 카드, 마이크 버튼)을
액자처럼 보여 주어 두 톤 차이를 의도된 대비로 읽히게 합니다.
